...
class RouteResponse(BaseModel):
    ...
    routes: List[RouteCandidate] = Field(..., description="List of route options available to the user")
    scores: Optional[List[RouteScores]] = Field(
        None, description="Optional list of comfort/safety scores per route"
    )
    metadata: ResponseMetadata = Field(
        default_factory=lambda: ResponseMetadata(
            timestamp=datetime.utcnow(), request_id="N/A", source="SafeNavCore"
        ),
